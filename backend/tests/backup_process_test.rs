...
use rdumper_backend::services::FilesystemBackupService;
use rdumper_backend::models::{DatabaseConfig, Task, BackupMetadata};
...
async fn test_backup_process_completes_successfully() {
    ...
    let file_name = backup_file.file_name().to_string_lossy();
